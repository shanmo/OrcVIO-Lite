...
namespace orcvio
{
  ...
  OrcVIO::OrcVIO(std::string &config_file_) : is_gravity_set(false),
                                              is_first_img(true),
                                              config_file(config_file_)
  {
      std::string filename = "/tmp/orcvio_debug_img_to_imu_mapping.txt";
      debug_img_to_imu_mapping_file_.open(filename);
      if (!debug_img_to_imu_mapping_file_.good()) {
          std::cerr << "Error opening file: " << filename << "\n";
      } else {
          debug_img_to_imu_mapping_file_.precision(7);
          debug_img_to_imu_mapping_file_ << std::fixed;
      }
      return;
  }
                               ...
                               std::vector<ImuData> &imu_msg_buffer)
  {
    ...
    if (if_FEJ_config && !if_FEJ && state_server.imu_state.time - take_off_stamp >= 0)
    {
      if_FEJ = true;
    }
    ...
    fImuState.precision(7);
    fImuState << std::fixed << state_server.imu_state.time << " "
              << px << " " << py << " " << pz << " "
              << qx << " " << qy << " " << qz << " " << qw << endl;
  ...
  void OrcVIO::batchImuProcessing(const double &time_bound,
                                  std::vector<ImuData> &imu_msg_buffer)
  {
    debug_img_to_imu_mapping_file_ << "\n" << time_bound << " ";
    ...
    {
      double imu_time = imu_msg.timeStampToSec;
      debug_img_to_imu_mapping_file_ << imu_time << " ";
                            ...
                            const Vector3d &m_acc)
  {
    ...
    if (!use_larvio_flag)
    {
      ...
      // we should always use acc in place of (a_m - b_a) in Joan Sola's expressions and gyro in place of (w_m - b_m)
  ...
  void OrcVIO::stateAugmentation()
  {
    ...
    IMUState_Aug &imu_state = state_server.imu_states_augment[state_server.imu_state.id];
      ...
      Matrix<double, 2, 3> &H_f, Vector2d &r)
  {
    ...
    dpc_dxe.leftCols(3) = (R_w2c * skewSymmetric(p_bf_w) * R_b2w) - (R_b2c * skewSymmetric(t_c_b));
                                             ...
                                             Eigen::Vector2d &r)
  {
    ...
    // Prepare all the required data.
    if (state_server.imu_states_augment.find(state_id) == state_server.imu_states_augment.end())
    ...
    if (state_server.imu_states_augment.find(feature.id_anchor) != state_server.imu_states_augment.end())
    {
    ...
    Vector3d p_baf_w = ((if_FEJ && !anch_is_nui) ? feature.position_FEJ - imu_state_a.position_FEJ : p_w - t_ba_w);
    Vector3d p_bkf_w = (if_FEJ ? feature.position_FEJ - imu_state_k.position_FEJ : p_w - t_bk_w);
                                       ...
                                       Eigen::MatrixXd &H_x, Eigen::VectorXd &r)
  {
    ...
    for (const auto &state_id : state_ids)
    {
      ...
      if (1 == feature_idp_dim &&
          state_id == feature.id_anchor)
        continue; // Measurement of anchor frame should not be ussed when applying 1d idp
    ...
    if (use_schmidt)
    {
      H_x = MatrixXd::Zero(jacobian_row_size,
                           LEG_DIM + 6 * state_server.imu_states_augment.size() + feature_idp_dim * state_server.feature_states.size() + 6 * state_server.nui_ids.size());
    }
    else
    {
      H_x = MatrixXd::Zero(jacobian_row_size,
                           LEG_DIM + 6 * state_server.imu_states_augment.size() + feature_idp_dim * state_server.feature_states.size());
    }
    ...
    if (use_schmidt)
    {
      int add = (LEG_DIM + 6 * state_server.imu_states_augment.size() + feature_idp_dim * state_server.feature_states.size() + 6 * state_server.nui_ids.size()) - state_server.state_cov.rows();
                                   ...
                                   Eigen::MatrixXd &H_x, Eigen::Vector2d &r)
  {
    ...
    int anchor_cntr;
    int anchor_index;
    int num_new = (LEG_DIM + 6 * state_server.imu_states_augment.size() + feature_idp_dim * state_server.feature_states.size() + 6 * state_server.nui_ids.size() - state_server.state_cov.rows()) / feature_idp_dim;
    if (use_schmidt &&
        find(state_server.nui_ids.begin(), state_server.nui_ids.end(), feature.id_anchor) != state_server.nui_ids.end())
    {
      ...
      anchor_index = LEG_DIM + 6 * state_server.imu_states_augment.size() + feature_idp_dim * (state_server.feature_states.size() - num_new) + 6 * anchor_cntr;
    }
      ...
      const MatrixXd &H, const VectorXd &r)
  {
         ...
         ++i, feature_itr++)
    {
      ...
      else if (state_server.imu_states_augment.find(map_server[feature_id].id_anchor) != state_server.imu_states_augment.end())
      ...
      const Eigen::MatrixXd &H_msckf, const Eigen::VectorXd &r_msckf)
  {
    ...
    H_o.block(
        ...
        H_ekf_new.rows() - sz_new, state_server.state_cov.cols()) = H_ekf_new.block(0, 0, H_ekf_new.rows() - sz_new, state_server.state_cov.cols());
    // H1
    MatrixXd H_1 = H_ekf_new.block(
        H_ekf_new.rows() - sz_new, 0, sz_new, state_server.state_cov.cols());
    // H2
    MatrixXd H_2 = H_ekf_new.block(
    ...
    r_o.segment(
        r_msckf.rows() + r_ekf.rows(), r_ekf_new.rows() - sz_new) = r_ekf_new.segment(0, r_ekf_new.rows() - sz_new);
    // r1
    VectorXd r_1 = r_ekf_new.segment(r_ekf_new.rows() - sz_new, sz_new);
    ...
    if (r_1.rows() > 0)
    {
      ...
      MatrixXd P22 = -nHHP * HH.transpose() + feature_observation_noise * (H22.ldlt().solve(MatrixXd::Identity(sz_new, sz_new)));
    ...
    for (int timestamp_count = 0; timestamp_count < total_obs_num; timestamp_count++)
    {
      ...
      if (it != cur_window_timestamps.end())
      {
        ...
        Hx.block(Hx_zs_last_row_index, LEG_DIM + 6 * pose_index, zs_num_frame, 6) =
            jacobian_wrt_sensor_state.block(fjac_zs_last_row_index, 0, zs_num_frame, 6) * dcampose_dimupose_mat;
        ...
        Hx.block(Hx_zs_last_row_index, LEG_DIM + 6 * pose_index, 4, 6) =
            jacobian_wrt_sensor_state.block(sum_of_zs_num + timestamp_count * 4, 0, 4, 6) * dcampose_dimupose_mat;
  ...
  void OrcVIO::removeLostFeatures()
  {
    ...
    if (msckf_feature_ids.size() == 0 && ekf_new_feature_ids.size() == 0 && ekf_feature_ids.size() == 0)
      return;
    ...
    if (!if_ZUPT)
    {
      ...
      MatrixXd H_ekf_new = MatrixXd::Zero(jacobian_row_size_ekf_new,
                                          state_server.state_cov.cols() + feature_idp_dim * ekf_new_feature_ids.size());
  ...
  void OrcVIO::pruneImuStateBuffer()
  {
    ...
    for (auto &item : map_server)
    {
      ...
      if (feature.in_state)
      {
        ...
        if (it != involved_state_ids.end())
        {
          // if (use_schmidt) {
          if (use_schmidt &&
              state_server.imu_state.id - feature.id_anchor > 2)
          { // Only use mature pose as nuisance state, the threshold '2' is decide by criterion in findRedundantImuStates
            ...
            if (find(new_nui_state_ids.begin(), new_nui_state_ids.end(), feature.id_anchor) == new_nui_state_ids.end())
      ...
      else
      {
        if (feature.is_initialized)
        {
          ...
          {
            if (0)
            { // NOTE: Potential EKF features does not produce nuisance state
            ...
            involved_state_ids.size() > 1)
        {
          ...
          bool if_tracked = feature.observations.find(state_server.imu_state.id) != feature.observations.end();
          if (!feature.is_initialized)
          {
    ...
    if (!if_ZUPT && used_IDs.size() != 0)
    {
      ...
      for (auto &item : map_server)
      {
        ...
        if (find(used_IDs.begin(), used_IDs.end(), feature.id) != used_IDs.end())
        {
          // debug log
          if (involved_state_ids.size() == 0)
    ...
    for (const auto &state_id : rm_imu_state_ids)
    {
      ...
      if (use_schmidt && find(new_nui_state_ids.begin(), new_nui_state_ids.end(), state_id) != new_nui_state_ids.end())
      {
  ...
  bool OrcVIO::checkZUPTFeat()
  {
    ...
    // default
    float feature_displacement_threshold = 20;
    if (coarse_feature_dis.empty() || coarse_feature_dis.size() < feature_displacement_threshold)
    {
  ...
  bool OrcVIO::checkZUPTIMU()
  {
    ...
    {
      printf("[ZUPT]: There are no IMU data to check for zero velocity with!!\n");
      return do_zupt;
    }
  ...
  void OrcVIO::measurementUpdate_ZUPT_vpq()
  {
    ...
    // r_p
    const Vector3d &p_curr = state_server.imu_states_augment[state_server.imu_state.id].position;
    const Vector3d &p_prev = state_server.imu_states_augment[state_server.imu_state.id - 1].position;
    r.segment<3>(3) = -(p_curr - p_prev);

    const Vector4d &q_c = rotationToQuaternion(state_server.imu_states_augment[state_server.imu_state.id].orientation);
    ...
    const Vector4d &q_p = rotationToQuaternion(state_server.imu_states_augment[state_server.imu_state.id - 1].orientation);
    ...
    for (const auto &feature_id : lost_ids)
    {
      ...
      if (use_schmidt)
      {
        ...
        if (find(state_server.nui_ids.begin(),
                 state_server.nui_ids.end(), id_an) != state_server.nui_ids.end())
        {
  ...
  StateIDType OrcVIO::getNewAnchorId(Feature &feature, const std::vector<StateIDType> &rmIDs)
  {
    ...
    {
      bool bObs = (feature.observations.find(key_state_iter->first) != feature.observations.end());
      if (!bObs)
        continue;
                                ...
                                const Eigen::Vector3d &acc_old, const Eigen::Vector3d &gyro_old)
  {
    ...
    if (use_larvio_flag || use_left_perturbation_flag)
    {
      ...
      // Simple blocks
      Vector3d vk = (if_FEJ ? state_server.imu_state_FEJ_old.velocity : state_server.imu_state_old.velocity);
      Vector3d pk = (if_FEJ ? state_server.imu_state_FEJ_old.position : state_server.imu_state_old.position);
      Vector3d vkp1 = (if_FEJ ? state_server.imu_state_FEJ_now.velocity : state_server.imu_state.velocity);
      Vector3d pkp1 = (if_FEJ ? state_server.imu_state_FEJ_now.position : state_server.imu_state.position);
      ...
      if (calib_imu)
      {
        ...
        Matrix3d kvM1_2 = R_mid * FL_kh + skewSymmetric(R_mid * acc_mid) * dtime * kqM1_1 / 2;
        Matrix3d kvM1_3 = R_mid * FL_kh + skewSymmetric(R_mid * acc_mid) * dtime * kqM1_2 / 2;
        Matrix3d kvM1_4 = R_kp1 * FL_kp1 + skewSymmetric(R_kp1 * acc) * RFL_;
        Matrix3d vM1_ = dtime * (kvM1_1 + 2 * kvM1_2 + 2 * kvM1_3 + kvM1_4) / 6;
        Phi.block<3, 3>(3, 40) = // Phi_v_M1
            C_bk2w * vM1_;
        Matrix3d kvM2_1 = FD_k;
        Matrix3d kvM2_2 = R_mid * FD_kh + skewSymmetric(R_mid * acc_mid) * dtime * kqM2_1 / 2;
        Matrix3d kvM2_3 = R_mid * FD_kh + skewSymmetric(R_mid * acc_mid) * dtime * kqM2_2 / 2;
        Matrix3d kvM2_4 = R_kp1 * FD_kp1 + skewSymmetric(R_kp1 * acc) * RFD_;
  ...
  void OrcVIO::rmUselessNuisanceState()
  {
    ...
    for (int i = 0; i < rm_ids.size(); ++i)
    {
      ...
      int nui_state_start = LEG_DIM + 6 * state_server.imu_states_augment.size() + feature_idp_dim * state_server.feature_states.size() + 6 * nui_sequence;
  ...
  void OrcVIO::incrementState_IMUCam(const VectorXd &delta_x)
  {
    ...
    // // ignore large update, only do this when using zupt
    // if ((
    //     //  delta_x_legacy.segment<3>(3).norm() > 3.0 || 
    //      delta_x_legacy.segment<3>(6).norm() > 5.0    
    //     ) &&
    //     discard_large_update_flag && if_ZUPT_valid)
    // {
