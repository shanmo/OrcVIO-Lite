...
void RefineGravity(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x, const Vector3d& TIC)
{
...
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x, const Vector3d& TIC)
{
    int all_frame_count = all_image_frame.size();
...
bool VisualIMUAlignment(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs, Vector3d &g, VectorXd &x, const Vector3d& TIC)
{
    ...
    if(LinearAlignment(all_image_frame, g, x, TIC))     
        return true;
    else
        return false;
}
