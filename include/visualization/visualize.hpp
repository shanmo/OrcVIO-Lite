...
void DrawGTPose(pangolin::OpenGlMatrix &Twc) {
    const float &w = 0.08;
    const float h = w*0.75;
    const float z = w*0.6;
...
template <typename T>
void DrawGTPoses(T begin, T end) {
    uint64_t count = 0;
    for (auto it = begin; it != end; ++it, ++count) {
        if (count % 20 == 0) {
            pangolin::OpenGlMatrix gt_pose_pgl;
            std::copy(it->data(),
                      it->data() + 16,
                      gt_pose_pgl.m);
            DrawGTPose(gt_pose_pgl);
        }
    }
}
...
void DrawActiveMapPoints(const std::map<orcvio::FeatureIDType,Eigen::Vector3d>& mMapPoints) {
    if (mMapPoints.empty())
        return;
...
void DrawStableMapPoints(const std::map<orcvio::FeatureIDType,Eigen::Vector3d>& mMapPoints) {
    if (mMapPoints.empty())
        return;
