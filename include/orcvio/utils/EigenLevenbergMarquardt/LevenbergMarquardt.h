...
#ifndef EIGENLEVENBERGMARQUARDT_LEVENBERGMARQUARDT_H
#define EIGENLEVENBERGMARQUARDT_LEVENBERGMARQUARDT_H

#include <iostream>

namespace EigenLevenbergMarquardt {
...
template <typename _Scalar, int NX=Dynamic, int NY=Dynamic,
  typename _InputType=Matrix<_Scalar, NX, 1> >
struct DenseFunctor
{
  ...
  typedef Matrix<Scalar,InputsAtCompileTime,1> DiffType;
  typedef Matrix<Scalar,ValuesAtCompileTime,1> ValueType;
  typedef Matrix<Scalar,ValuesAtCompileTime,InputsAtCompileTime> JacobianType;

  // There are three QR decomposition classes: HouseholderQR (no pivoting, so fast but unstable), 
  ...
  virtual int operator()(const InputType &x, Eigen::Ref<ValueType> fvec) const = 0;
  // should be defined in derived classes

  virtual int df(const InputType &x, Eigen::Ref<JacobianType> fjac) const = 0;
  // should be defined in derived classes

  // should be defined in derived classes
  template <typename T>
  Scalar scaled_norm(const T& D, const T& x) {
...
/**
  ...
  * www.mcs.anl.gov/~more/ANL8074a.pdf
  */
...
class LevenbergMarquardt
{
  public:
    ...
    typedef typename FunctorType::InputType FInputType;
    typedef typename FunctorType::DiffType FDiffType;
    typedef typename FunctorType::ValueType FValueType;
  ...
  public:
    ...
    LevenbergMarquardtSpace::Status minimize(FInputType &x);
    LevenbergMarquardtSpace::Status minimizeInit(FInputType &x);
    LevenbergMarquardtSpace::Status minimizeOneStep(FInputType &x);
  ...
  private:
    ...
    FDiffType m_wa1, m_wa2, m_wa3; //Temporary vectors
    FValueType m_wa4; // Temporary vector
    FInputType m_input_wa1;
    RealScalar m_par;
...
LevenbergMarquardt<FunctorType>::minimize(FInputType  &x)
{
    ...
    do {
        // std::cout << " x: " << x << "\n";
        if (DEBUG)
          std::cout << " f(x): " << m_fvec.array().square().sum() << "\n";
...
template<typename FunctorType>
LevenbergMarquardtSpace::Status
  LevenbergMarquardt<FunctorType>::minimizeInit(FInputType  &x)
{
    n = x.size();
    m = m_functor.values();
