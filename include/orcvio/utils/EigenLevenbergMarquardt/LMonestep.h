...
#ifndef EIGENLEVENBERGMARQUARDT_LMONESTEP_H
#define EIGENLEVENBERGMARQUARDT_LMONESTEP_H
...
EigenLevenbergMarquardt::LevenbergMarquardtSpace::Status
LevenbergMarquardt<FunctorType>::minimizeOneStep(FInputType  &x)
{
  using std::abs;
  using std::sqrt;
  ...
  // std::cout << "[Object LM] check object lm input size" << std::endl;
  // std::cout << "[Object LM] m_fjac size " << m_fjac.rows() << " x " << m_fjac.cols() << std::endl; 
  ...
  if (m_iter == 1) {
      ...
      xnorm = m_functor.scaled_norm(m_diag, x);
  ...
  /* m_qtf. */
  // solve min_p || f + Jp || s.t. ||D p || < delta
  // p = J \ f
  m_wa4 = m_fvec;
  ...
  do {
    ...
    m_wa1 = -m_wa1;
    m_input_wa1 = x + m_wa1;
    pnorm = m_diag.cwiseProduct(m_wa1).stableNorm(); // || D p ||
    ...
    if ( m_functor(m_input_wa1, m_wa4) < 0)
        return LevenbergMarquardtSpace::UserAsked;
    ++m_nfev;
    ...
    {
      m_info = Success;
      // std::cout << "pnorm(" << pnorm << "); m_delta(" << m_delta << ") < m_xtol(" << m_xtol << ") * xnorm(" << xnorm << ") \n" ;
