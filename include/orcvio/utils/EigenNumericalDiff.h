...
#ifndef EIGENNUMERICALDIFF_H
#define EIGENNUMERICALDIFF_H

namespace EigenNumericalDiff { 
...
public:
    ...
    typedef typename Functor::DiffType DiffType;
    ...
    int df(const InputType& _x, JacobianType &jac) const
    {
        ...
        for (int j = 0; j < n; ++j) {
            ...
            DiffType dx(x.size()); dx.setZero();
            dx[j] = h;
            switch(mode) {
                case Forward:
                    x += dx;
                    Functor::operator()(x, val2);
                    // assert(val2.allFinite());
                    nfev++;
                    x = _x;
                    jac.col(j) = (val2-val1)/h;
                    // assert(!Eigen::isnan(jac.col(j).array()).any());
                    // assert(!Eigen::isinf(jac.col(j).array()).any());
                    // assert(jac.col(j).allFinite());
                    break;
                case Central:
                    x += dx;
